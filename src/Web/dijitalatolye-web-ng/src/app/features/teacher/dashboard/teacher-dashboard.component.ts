    ...
    @if (loading()) {
      ...
      <div class="max-w-6xl">
        <header class="mb-8 flex items-center justify-between">
          <div>
            <h1 class="text-2xl font-extrabold text-slate-900">Kontrol Paneli</h1>
            <p class="text-sm text-slate-600 mt-1">İçeriklerinize ait güncel durum özeti.</p>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div class="bg-white rounded-2xl border border-slate-200 p-6 flex items-center gap-5 shadow-sm">
            ...
            <div>
              <p class="text-sm font-medium text-slate-500">Yayında</p>
              <p class="text-3xl font-extrabold text-slate-900 mt-1">{{ publishedCount() }}</p>
            </div>
          </div>
          <div class="bg-white rounded-2xl border border-slate-200 p-6 flex items-center gap-5 shadow-sm">
            ...
            <div>
              <p class="text-sm font-medium text-slate-500">İncelemede</p>
              <p class="text-3xl font-extrabold text-slate-900 mt-1">{{ pendingCount() }}</p>
            </div>
          </div>
          <div class="bg-white rounded-2xl border border-slate-200 p-6 flex items-center gap-5 shadow-sm">
            <div class="w-14 h-14 rounded-xl flex items-center justify-center bg-slate-50 text-slate-600">
              <mat-icon style="font-size:28px;width:28px;height:28px">description</mat-icon>
            </div>
            <div>
              <p class="text-sm font-medium text-slate-500">Taslak / Revizyon</p>
              <p class="text-3xl font-extrabold text-slate-900 mt-1">{{ draftCount() }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div class="divide-y divide-slate-100">
            @if (contents().length === 0) {
              <div class="p-8 text-center text-slate-500"><p>Henüz hiç içerik yüklemediniz.</p></div>
            } @else {
                ...
                <div class="px-6 py-4 flex items-center justify-between hover:bg-slate-50 transition">
                  ...
                  <div class="flex items-center gap-4">
                    ...
                    @if (isPublished(c.state)) {
                      <a [routerLink]="['/play', c.id]" class="text-slate-400 hover:text-brand-600 transition">
...
export class TeacherDashboardComponent implements OnInit {
  ...
  badgeClass(state: ContentState): string {
    ...
    if (['Draft', 'RevisionRequested'].includes(state)) return base + 'bg-slate-100 text-slate-700 border border-slate-200';
