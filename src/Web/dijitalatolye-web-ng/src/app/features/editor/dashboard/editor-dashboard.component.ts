...
@Component({
  ...
  template: `
    <header class="mb-6">
      <h1 class="text-2xl font-extrabold text-ink">Editör Paneli</h1>
      <p class="text-sm text-muted mt-1">İnceleme bekleyen içerikler ve kişisel performansınız.</p>
    </header>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      <div class="rounded-2xl bg-surface border border-line/10 p-5">
        ...
        <div class="mt-3 text-2xl font-extrabold text-ink">{{ stats()?.pendingQueue ?? '—' }}</div>
        <div class="text-xs text-dim">Beklemede</div>
      </div>
      <div class="rounded-2xl bg-surface border border-line/10 p-5">
        ...
        <div class="mt-3 text-2xl font-extrabold text-ink">{{ stats()?.reviewedToday ?? '—' }}</div>
        <div class="text-xs text-dim">Bugün incelenen</div>
      </div>
      <div class="rounded-2xl bg-surface border border-line/10 p-5">
        ...
        <div class="mt-3 text-2xl font-extrabold text-ink">{{ stats()?.approvedThisWeek ?? '—' }}</div>
        <div class="text-xs text-dim">Onaylanan (hafta)</div>
      </div>
      <div class="rounded-2xl bg-surface border border-line/10 p-5">
        ...
        <div class="mt-3 text-2xl font-extrabold text-ink">{{ stats()?.rejectedThisWeek ?? '—' }}</div>
        <div class="text-xs text-dim">Reddedilen (hafta)</div>
      </div>
    </div>
    ...
    <div class="rounded-2xl bg-gradient-to-br from-brand-600 via-brand-700 to-brand-800 text-white p-8 shadow-lg shadow-brand-900/20">
      ...
      <a routerLink="/editor/queue"
        class="mt-4 inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-surface text-brand-700 font-semibold hover:bg-brand-50">
    ...
    <div class="mt-8 grid md:grid-cols-2 gap-5">
      <a routerLink="/editor/history" class="group rounded-2xl bg-surface border border-line/10 p-6 hover:border-brand-300 hover:shadow-md transition">
        ...
        <p class="mt-1 text-sm text-muted">Verdiğiniz tüm kararlar, gerekçeler ve yeniden inceleme talepleri.</p>
      </a>
    </div>
  `,
})
