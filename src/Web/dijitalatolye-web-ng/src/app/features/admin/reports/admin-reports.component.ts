  ...
  template: `
    <div>
      <header class="mb-6 flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-extrabold text-slate-900">Raporlar & Metrikler</h1>
          <p class="text-sm text-slate-600 mt-1">Platform sağlığı, kullanım istatistikleri ve içerik metrikleri.</p>
        </div>
        <select class="text-sm rounded-lg border border-slate-200 px-3 py-2 bg-white focus:border-brand-400 focus:ring-2 focus:ring-brand-100 outline-none">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div class="rounded-2xl bg-white border border-slate-200 p-5">
          ...
          <div class="mt-3 text-2xl font-extrabold text-slate-900">{{ data()?.activeUsers ?? '—' }}</div>
          <div class="text-xs text-slate-500">Aktif kullanıcı</div>
        </div>
        <div class="rounded-2xl bg-white border border-slate-200 p-5">
          <div class="flex items-center justify-between">
          ...
          <div class="mt-3 text-2xl font-extrabold text-slate-900">{{ data()?.publishedContents ?? '—' }}</div>
          <div class="text-xs text-slate-500">Yayınlanan içerik</div>
        </div>
        <div class="rounded-2xl bg-white border border-slate-200 p-5">
          <div class="flex items-center justify-between">
          ...
          <div class="mt-3 text-2xl font-extrabold text-slate-900">{{ data()?.totalPlays ?? '—' }}</div>
          <div class="text-xs text-slate-500">Toplam oynatma</div>
        </div>
        <div class="rounded-2xl bg-white border border-slate-200 p-5">
          <div class="flex items-center justify-between">
          ...
          <div class="mt-3 text-2xl font-extrabold text-slate-900">{{ data()?.aiApprovalRate ?? '—' }}</div>
          <div class="text-xs text-slate-500">AI onay oranı</div>
        </div>
      </div>
      ...
      <div class="grid lg:grid-cols-3 gap-5">
        <div class="lg:col-span-2 rounded-2xl bg-white border border-slate-200 p-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-xs text-slate-500">Son 30 gün</span>
          </div>
          <div class="h-64 rounded-xl bg-gradient-to-br from-brand-50 to-accent-50 border border-brand-100 flex items-center justify-center text-slate-500 text-sm">
            Grafik verisi henüz bağlanmadı (Prometheus / Reports endpoint).
          </div>
        </div>

        <div class="rounded-2xl bg-white border border-slate-200 p-6">
          <h2 class="font-semibold text-slate-900 mb-4">En aktif öğretmenler</h2>
          <ul class="space-y-3 text-sm">
            @for (t of data()?.topTeachers ?? []; track t.name; let i = $index) {
              <li class="flex items-center justify-between">
                ...
                <span class="text-xs text-slate-500">{{ t.contents }} içerik</span>
              </li>
            }
            @if (!data()) { <li class="text-sm text-slate-500">Yükleniyor...</li> }
          </ul>
        </div>
      </div>
